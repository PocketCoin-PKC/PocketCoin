...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "PocketcoinNetwork") type = PocketcoinNetwork;
    if(strType == "PocketcoinNetworkVariable") type = PocketcoinNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == PocketcoinNetwork) s = "PocketcoinNetwork";
    if(type == PocketcoinNetworkVariable) s = "PocketcoinNetworkVariable";
